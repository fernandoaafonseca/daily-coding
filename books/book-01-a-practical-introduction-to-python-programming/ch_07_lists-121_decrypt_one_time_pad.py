...
def main():
	...
	display_result(encrypted_message, shifts, decrypted_message)
...
def get_message() -> str:
	while True:
		try:
			...
			message = str(input('Enter a message to be decrypted: ')).lower()
...
def get_shifts_from_user() -> list[int]:
	while True:
		try:
			press_any_key_to_continue()
			clear_terminal()

			shifts_from_user = str(input('Enter a list of integers separated by commas (like "1, 2, 3"): '))
			# Generates a list of "ints" splitting the string by "commas"
			shifts = [int(item) for item in shifts_from_user.split(',')]
...
def display_result(encrypted_message: str, shifts: list[int], decrypted_message: str) -> None:
	press_any_key_to_continue()
	clear_terminal()
