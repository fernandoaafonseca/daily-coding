...
def main() -> None:
    ...
    decrypted_str = decrypt_rail_fence_three(user_string)
...
def decrypt_rail_fence_three(user_string: str) -> str:
    '''
    Decrypts a 3-rail Rail Fence Cipher.
    It reverses the process by reconstructing the order of indices.
    '''
    length = len(user_string)
    rail_len = [length // 3 + (1 if i < length % 3 else 0) for i in range(3)]

    # Split user_string into its three rails
    rails = []
    index = 0
    for l in rail_len:
